...
f.map! do |line|
  ...
  loop do 
    new_combo = []
    iterating_springs = springs
    contiguous_sizes.each do |num|
      
      idx = 0
      while idx + num < iterating_springs.length
        str = iterating_springs[idx, num]
        if !str.include?('.')
          iterating_springs = iterating_springs[idx + (num + 1)..]
          break
        end

        idx += 1
        no_arrangements = true if idx + num >= iterating_springs.length
      end
